...
const BACKEND_SCRIPT = path.resolve(__dirname, '..', 'examples', 'backend-echo.js');
...
function spawnBackend(port) {
    ...
    child.on('exit', (code) => {
        if (code !== 0 && code !== null) {
            console.error(`[start] demo backend on port ${port} exited with code ${code}`);
        }
    });
